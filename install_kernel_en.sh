...
function listInstalledLinuxKernel(){
	...
	if [[ "${osRelease}" == "debian" || "${osRelease}" == "ubuntu" ]]; then
        ...
        red " Uninstall kernel command1 apt remove -y --purge linux-xxx名称"
        red " Uninstall kernel command2 apt autoremove -y --purge linux-xxx名称"

	elif [[ "${osRelease}" == "centos" ]]; then
        ${sudoCmd} rpm -qa | grep kernel
        echo
        red " 如安装内核遇到kernel kernel-headers kernel-devel版本不一致问题, please manually uninstall the installed kernel"
        red " Uninstall kernel command rpm --nodeps -e kernel-xxx名称"
	fi
    green " =================================================="
    echo
}
