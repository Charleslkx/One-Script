...
allowPort() {
    ...
    # Debian/Ubuntu缺少iptables/netfilter-persistent时自动安装
    if [[ "${release}" == "debian" ]] || [[ "${release}" == "ubuntu" ]]; then
        if ! command -v iptables >/dev/null 2>&1; then
            echoContent yellow " ---> 未检测到iptables，正在安装"
            ${installType} iptables >/dev/null 2>&1
        fi
        if ! command -v netfilter-persistent >/dev/null 2>&1; then
            echoContent yellow " ---> 未检测到netfilter-persistent，正在安装"
    ...
    elif command -v iptables >/dev/null 2>&1 && command -v netfilter-persistent >/dev/null 2>&1 \
        && dpkg -l | grep -q "^[[:space:]]*ii[[:space:]]\+netfilter-persistent" \
...
checkAndConfigureLocale() {
    ...
    if [[ "${localeConfigured}" == "true" ]]; then
        echoContent green " ---> locale配置文件已存在: C.UTF-8"
    ...
    if [[ -n "${currentLang}" ]] && hasLocale "${currentLang}"; then
         if [[ "${currentLang}" == "C.UTF-8" ]] || [[ "${currentLang}" == "C.utf8" ]]; then
            echoContent green " ---> locale已正确配置: ${currentLang}"
            return 0
        fi
    fi
    
    echoContent yellow " ---> 检测到locale配置问题，正在修复..."
    
    # 针对Debian/Ubuntu系统
    if [[ "${release}" == "debian" ]] || [[ "${release}" == "ubuntu" ]]; then
        # 检查并安装locales包
        if ! dpkg -l | grep -q "^ii.*locales"; then
            echoContent green " ---> 安装locales包"
            ${installType} locales >/dev/null 2>&1
        fi
        
        # 生成常用的UTF-8 locales
        echoContent green " ---> 生成UTF-8 locale"
        ...
        # 选择可用的UTF-8 locale（优先C.UTF-8）
        local preferredLocales=("C.UTF-8" "en_US.UTF-8")
        for loc in "${preferredLocales[@]}"; do
            if hasLocale "${loc}"; then
                effectiveLocale="${loc}"
                break
            fi
        done
...
EOF
    ...
    # 针对CentOS/RHEL系统
        ...
        echoContent green " ---> 配置CentOS locale"
        ...
        # 设置系统locale（优先C.UTF-8）
        effectiveLocale=""
        if hasLocale "C.UTF-8"; then
            effectiveLocale="C.UTF-8"
        elif hasLocale "en_US.UTF-8"; then
            effectiveLocale="en_US.UTF-8"
        else
            # Try installing langpack-en for basic support checks? Or just default to C.UTF-8
             ${installType} glibc-langpack-en >/dev/null 2>&1 || true
             if hasLocale "en_US.UTF-8"; then
                effectiveLocale="en_US.UTF-8"
             else
                effectiveLocale="C.UTF-8" # force C.UTF-8
             fi
        fi
...
EOF
    ...
    # 验证locale是否配置成功
    if locale -a 2>/dev/null | grep -qi "C.UTF-8\|C.utf8\|en_US.utf8\|en_US.UTF-8"; then
        echoContent green " ---> locale配置成功"
        echoContent yellow " ---> 注意: 完整生效需要重新登录或重启系统"
        if [[ "${release}" == "debian" ]]; then
            echoContent skyBlue " ---> 是否立刻重启终端以避免locale警告？(Y/n)"
            read -r restartTerminal
            if [[ -z "${restartTerminal}" ]] || [[ "${restartTerminal}" =~ ^[Yy]$ ]]; then
                echoContent yellow " ---> 请重新登录或关闭并重新打开终端后继续"
                exit 0
            fi
        fi
    else
        echoContent yellow " ---> locale配置完成，但可能需要重启系统才能完全生效"
    fi
}
...
checkEssentialPackages() {
    ...
    if [[ "${release}" == "debian" ]] || [[ "${release}" == "ubuntu" ]]; then
        ...
        if ! dpkg -l | grep -q "^ii.*debconf-utils"; then
    ...
    if [[ ${#essentialPackages[@]} -gt 0 ]]; then
        echoContent yellow " ---> 检测到精简系统，安装必要组件: ${essentialPackages[*]}"
        for package in "${essentialPackages[@]}"; do
            echoContent green " ---> 安装 ${package}"
            ${installType} "${package}" >/dev/null 2>&1
        done
    else
        echoContent green " ---> 必要系统组件检查完成"
    fi
}
    ...
    if [[ "${release}" == "debian" ]] || [[ "${release}" == "ubuntu" ]]; then
        if ! dpkg -l | grep -q iptables-persistent; then
            ...
            if dpkg -l | grep -q "^ii.*iptables-persistent"; then
                echoContent green " ---> iptables-persistent安装成功"
                
                # 确保netfilter-persistent服务启动
                if command -v systemctl >/dev/null 2>&1; then
                    systemctl enable netfilter-persistent >/dev/null 2>&1
                    systemctl start netfilter-persistent >/dev/null 2>&1
                fi
            else
                echoContent yellow " ---> iptables-persistent安装可能未完全成功，但不影响继续"
            fi
...
promptDailyRebootCron() {
    ...
    if crontab -l 2>/dev/null | grep -q "0 21 \* \* \* /sbin/reboot"; then
        echoContent yellow " ---> 定时重启任务已存在，跳过询问"
        return
    fi

    # 交互式询问
    echoContent skyBlue "==========================================="
    echoContent skyBlue "  是否设置系统定时重启任务？"
    echoContent skyBlue "==========================================="
    echoContent yellow "  将在每天北京时间 05:00（UTC 21:00）自动重启"
    echoContent yellow "  以保持系统性能和释放资源"
    echo
    echoContent green "  1) 是 - 设置定时重启任务"
    echoContent green "  2) 否 - 跳过（不推荐用于长期运行）"
    echo
    read -r -p "请输入您的选择 [1-2]: " cron_choice
    ...
    case "$cron_choice" in
        1)
            ...
            ;;
        2|*)
            echoContent yellow " ---> 已跳过定时重启任务设置"
            echoContent yellow " ---> 注意：长期运行建议设置定时重启以保持系统性能"
            echoContent skyBlue " ---> 如需稍后设置，可重新运行本脚本"
            ;;
    esac
}

# 添加每天凌晨5点重启的定时任务（UTC 21:00 = UTC+8 05:00）
installDailyRebootCron() {
    # 检查是否已存在重启任务
    if crontab -l 2>/dev/null | grep -q "0 21 \* \* \* /sbin/reboot"; then
        echoContent yellow " ---> 每日北京时间05:00（UTC 21:00）重启任务已存在，跳过添加"
        return
    fi
    ...
    # 添加重启任务到备份文件（UTC 21:00 = 北京时间 05:00）
    echo "0 21 * * * /sbin/reboot" >>/tmp/backup_crontab_reboot.cron
    ...
    # 验证任务是否添加成功
    if crontab -l 2>/dev/null | grep -q "0 21 \* \* \* /sbin/reboot"; then
        echoContent green " ---> 每日北京时间05:00（UTC 21:00）自动重启任务添加成功"
        echoContent yellow " ---> 系统将在每天北京时间05:00自动重启以保持最佳性能"
...
xrayCoreInstall() {
    ...
    echoContent skyBlue "\n功能 : 安装 BBR 内核/加速"
    echoContent red "\n=============================================================="
    read -r -p "是否安装 BBR 内核/加速? [y/n]:" installBBRStatus
    if [[ "${installBBRStatus}" == "y" ]]; then
        if [[ ! -f "install_kernel.sh" ]]; then
            wget -N --no-check-certificate "https://raw.githubusercontent.com/mack-a/v2ray-agent/master/install_kernel.sh"
            chmod 700 install_kernel.sh
        fi
        ./install_kernel.sh
    fi
}
...
menu() {
    cd "$HOME" || exit
    echoContent red "\n=============================================================="
    echoContent green "描述：All in One脚本\c"
    showInstallStatus
    checkWgetShowProgress
    echoContent red "\n=============================================================="
    if [[ -n "${coreInstallType}" ]]; then
