...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -O /etc/v2ray-agent/install.sh -N --no-check-certificate "https://raw.githubusercontent.com/charleslkx/one-script/master/install.sh"
    else
        wget -c -q "${wgetShowProgressStatus}" -O /etc/v2ray-agent/install.sh -N --no-check-certificate "https://raw.githubusercontent.com/charleslkx/one-script/master/install.sh"
    fi
    ...
    echoContent skyBlue "wget -O /root/install.sh -N --no-check-certificate https://raw.githubusercontent.com/charleslkx/one-script/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
