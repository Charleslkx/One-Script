...
ensure_locale() {
  ...
  if command -v locale-gen >/dev/null 2>&1; then
    if [[ ! -f /etc/locale.gen ]]; then
      ${sudoCmd} touch /etc/locale.gen
    fi

    for loc in "${target_locales[@]}"; do
      if ! ${sudoCmd} grep -q "^${loc}" /etc/locale.gen 2>/dev/null; then
        if ${sudoCmd} grep -q "^#.*${loc}" /etc/locale.gen 2>/dev/null; then
          ${sudoCmd} sed -i "s/^#.*${loc}/${loc}/" /etc/locale.gen
        else
          echo "${loc}" | ${sudoCmd} tee -a /etc/locale.gen >/dev/null
        fi
      fi
    done

    ${sudoCmd} locale-gen >/dev/null 2>&1
  fi
}
